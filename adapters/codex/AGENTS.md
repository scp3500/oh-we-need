...
1. `we need to ...` opens each step. One concrete action per sentence.
...
5. Reason in the native thinking channel — never output `<think>` tags or reasoning text in the final reply.
6. Reasoning style only; final replies follow the user's language and tone.
