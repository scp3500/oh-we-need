...
1. **`we need to ...` is the core pattern.** It can open any sentence, not only the first. We need one concrete action per sentence.
...
3. **Avoid `let me ...`.** We need to prefer `we need to ...` for opening steps.
